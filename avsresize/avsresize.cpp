...
namespace {
    ...
    constexpr int PLANE_ORDER_RGB[] = { PLANAR_R, PLANAR_G, PLANAR_B, PLANAR_A };
    constexpr int PLANE_ORDER_YUV[] = { PLANAR_Y, PLANAR_U, PLANAR_V, PLANAR_A };
            ...
            a(args)
        {
            ...
            else
            {
                if (a.use_props == 2 && (!a.colorsp_def ||
                    ...
                    !a.chromal_def ||
                    a.match_chrl[1] == "auto"))
                    g_saved_env->ThrowError("every value of colorspace_op and chromaloc_op must be specified for use_props=2");
                else if (a.use_props == 3 || a.use_props == 4)
                {
                    ...
                    if (a.use_props == 4 && g_saved_env->propNumElements(props, "z_Matrix") == -1)
                        g_saved_env->ThrowError("there are no z_xxx frame properties");
                        ...
                        g_saved_env->ThrowError("missing or unspec frame property");
                }
            ...
            if (!a.use_props || a.use_props == 2)
            {
                if (!params.interlaced)
                {
                    ...
                    m_tmp.reset(aligned_new(m_graph.get_tmp_size(), 64));
                }
            }
        }
        ...
        PVideoFrame __stdcall GetFrame(int n, IScriptEnvironment* env) override try
        {
            ...
            if (!a.use_props || a.use_props == 2)
            {
                ...
                if (p.interlaced)
                {
                    ...
                    m_tmp.reset(aligned_new(std::max(m_graph.get_tmp_size(), graph_b.get_tmp_size()), 64));
                ...
                else
                {
                    if (v8)
                    {
                        ...
                        // TODO: support interlaced video with frame properties.
                        if ((env->propNumElements(props, "_FieldBased") > -1 && env->propGetInt(props, "_FieldBased", 0, nullptr) > 0) ||
                            (env->propNumElements(props, "_Field") > -1 && env->propGetInt(props, "_Field", 0, nullptr) > 0))
...
#undef TYPE
    ...
    constexpr char CONVERT_FUNCTION_NAME[] = "z_ConvertFormat";

#define OPT(n, type, name) "[" #name "]" #type
    constexpr char CONVERT_SIGNATURE[] =
        "c"
        OPT(1, i, width)
        OPT(2, i, height)
        OPT(3, s, pixel_type)
...
#undef OPT
    ...
    AVSValue __cdecl create_resize(AVSValue args, void* user_data, IScriptEnvironment* env) try
    {
        ...
        if (a.use_props < -1 || a.use_props > 4)
            env->ThrowError("wrong use_props value");
        ...
        if (a.colorsp_def)
        {
            if (a.use_props < 4)
            {
                ...
                env->ThrowError("colorspace_op must be not specified when use_props=4");
        }
        ...
        if (a.chromal_def)
        {
            if (a.use_props < 4)
            {
                ...
                env->ThrowError("chromaloc_op must be not specified when use_props=4");
        }
        ...
        params.graph_params.scene_referred = args[22].AsBool(false);
